...
while (<$INPUT>) {
	...
	system("$SAMTOOLSEXE view -@ $THREAD_COUNT $BAMFILE $bamposition > $TEMPNAME");
	print "SAMTOOLS view...\t";
	my $editnuc = $fields[4];
	my $newmismatch = 0;
...
print "blatting reads (threads: $THREAD_COUNT)...\n";
my $pblat_params = "-threads=$THREAD_COUNT -stepSize=5 -repMatch=2253 -minScore=20 -minIdentity=0 -noHead $REFERENCEGENOME $fafile $pslfile";
system("$PBLAT_PATH $pblat_params");
system("rm $fafile");
#}
...
open(PSL, "<", $pslfile );
print "File handle PSL status: ", tell(PSL), "\n"; 
my %pslhash;
while(<PSL>) {
	chomp;
	my @pslfields = split;
